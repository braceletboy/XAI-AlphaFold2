    ...
    for orig_key in keys:
        ...
        if isinstance(item, Element):
            ...
            while not finished:
                ...
                try:
                    key = dict2.keys()[0]
                    dict2[key] = dictElement(
                        dict2[key], prefix, number_multiples)
                except:
                    finished = True
                else:
                    dict2 = dict3
...
def countBytes(arrays, base=False):
    ...
    *base* is **True**, use number of bytes from the base array."""
    ...
    if base:
        ...
        def nbytes(arr): return getbase(arr).nbytes
    else:
        def nbytes(arr): return arr.nbytes
...
def getDataPath(filename):
    if PY3K:
        ...
        path = importlib.util.find_spec(
            'prody.utilities.datafiles').submodule_search_locations[0]
        ...
        if j == 0:
            ...
            if chid != chainids[j-1]:
                ...
                if chid_alt >= 10**num_chars:
                    num_chars += 1
                    new_chainids = array(
                        new_chainids, dtype=new_chainids.dtype.str[:2]+str(num_chars))
